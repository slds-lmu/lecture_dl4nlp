...
no longer seen as valid.
	\end{itemize}
	\item Training an LLM on less than 200 billion tokens is not recommended
...
\begin{itemize}
        \item bf16 has a larger dynamic range than fp16
 	(8 exponent bits instead of 5)
        \item bf16 is less precise, but also less likely to
...
\textit{It's hard to fit meaningful floats into 8 bits, but
you can use   integer quantization: INT8.}
...
\begin{vbframe}{Zero-point INT8 (asymmetric)}
	...
	\includegraphics[width = 10cm]{./figure/zero,point,maarten.png} 
...
\item We assume: $min(\mathbf{X}) \leq 0 \leq max(\mathbf{X})$
            ...
            \item $X_{quant} = round(\text{scale}\cdot \mathbf{X} + \text{zeropoint})$\\
             ...
             = $round(\text{scale}\cdot (\mathbf{X}  - min(\mathbf{X}))) - 128$\\
