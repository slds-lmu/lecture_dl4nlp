...
\begin{itemize}
	...
	\begin{itemize}
	    \item This was true for Chinchilla
	models \citebutton{Hoffmann et al.,
	2022}{https://arxiv.org/abs/2203.15556},\\ not clear
	to what extent it still holds.
	\end{itemize}
...
\begin{itemize}
\item Note that we assume: $min(\mathbf{X}) \leq 0 \leq max(\mathbf{X})$
            \item $\text{scale} = \frac{255}{max(\mathbf{X}) - min(\mathbf{X})}$
            \item $\text{zeropoint} = -round(\text{scale}\cdot min(\mathbf{X})) - 128$
            \item $X_{quant} = round(\text{scale}\cdot \mathbf{X} + \text{zeropoint})$
            \item $X_{quant}(0) = zeropoint$
            \item $X_{quant} = round(\text{scale}\cdot \mathbf{X} + \text{zeropoint})$
            \item = $round(\text{scale}\cdot \mathbf{X} + -round(\text{scale}\cdot min(\mathbf{X}) - 128))$
            \item = $round(\text{scale}\cdot (\mathbf{X}  - min(\mathbf{X}))) - 128$
            \item = $round(255 \cdot \frac{\mathbf{X} -
