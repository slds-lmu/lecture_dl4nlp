...
\begin{vbframe}{restore lecture command}\end{vbframe}
...
\begin{itemize}
	...
	\end{itemize}
    \item In the backward pass at each layer, gradients have to be calculated for the weights at that layer and for the previous layers' output, so that the gradient of the previous layers' weights can be calculated
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{We want to fix $P$ and $D$, so that we
	get the best possible performance given an available compute budget.}
	...
	2022}{https://arxiv.org/abs/2203.15556},\\but does
	not hold for all LLMs
	\end{itemize}
	\item Training a LLM on less than 200 billion tokens is not recommended
	\item Rule of thumb: First determine the uppermost inference cost, and then train the biggest model within that boundary
