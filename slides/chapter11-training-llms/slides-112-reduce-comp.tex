...
\begin{itemize}
\item Training LLMs on several GPUs is faster than on one,
  but we need to figure out how to distribute.
 	\item Avoiding OOM (out of memory) issues
...
\begin{itemize}
    \item \textbf{Data Splitting}
        \begin{itemize}
            \item Dataset  divided into smaller chunks. Each chunk  assigned to a different GPU.
            \item Each node processes a different subset of
              the data in parallel.
        \end{itemize}
    \item \textbf{Model Replication}
        \begin{itemize}
            \item Each GPU has a replica of the neural network model
            \item These replicas are trained independently on their respective data subsets
        \end{itemize}
    \item \textbf{Gradient Aggregation (All Reduce)}
        \begin{itemize}
            \item Gradients are computed locally on each
              node and then averaged across nodes.
        \end{itemize}
    \item \textbf{Parameter Synchronization}
        \begin{itemize}
            \item Model parameters are updated synchronously across  nodes.
...
\begin{vbframe}{Tensor Parallelism (1)}

\vfill

\begin{figure}
	\centering
	\includegraphics[width = 6cm]{./figure/tensor,parallelism,new.png} \\ 
...
%	\end{itemize}
	...
	\begin{itemize}
	\item Reducing memory requirements from $O(\MakeLowercase{n}^2)$
          to $O(\MakeLowercase{n})$ 
\ques{Why quadratic?}
	\end{itemize}
	\item IO aware
	\begin{itemize}
	...
	\end{itemize}
	\item Therefore: much faster than the regular implementation
	\begin{itemize}
		\item The bigger the attention matrices, the
                  more you gain (until
                  $O(\MakeLowercase{n})$ no longer fits).
	\end{itemize}
	\item Exact
	\begin{itemize}
...
\begin{itemize}
	\item GPU computations are extremely fast, but data
          transfer is slow.
		\item Typical GPU computations are elementwise
                  operations with a
huge number of elements, resulting in
                  high memory access
	\item $\rightarrow$ GPU has to wait for data.
	\item $\rightarrow$ Transformer operations are memory-bound.
...
\begin{vbframe}{Flash Attention}

\vfill

\begin{itemize}
		\item Tiling or chunking of matrices into blocks
		\item Operation fusion
                  \begin{itemize}
                  \item For example: first matrix
                    multiplication (query-key), softmax, second matrix
                    multiplication (weight-value)
\end{itemize}
                \item This minimizes redundant
                  swapping in and out of data
                  \item General idea: you avoid that the large intermediate
                    tensors (e.g., pre-softmax)  are
                    written to HBM and then read back into
                    SRAM memory
\end{itemize}
...
\begin{vbframe}{SRAM vs HBM / Fused Kernel}
...
\begin{vbframe}{Tiling}

\vfill
\begin{minipage}[c]{\textwidth}
	\begin{figure}
	\centering
	\includegraphics[width = 11cm]{./figure/tiling} \\ 
\end{figure}
\end{minipage}
...
\begin{itemize}
	\item FlashAttention requires implementing ``custom'' attention in CUDA
	\begin{itemize}
		\item A new CUDA kernel for each new attention implementation
		\item CUDA is lower-level than PyTorch
		\item Implementation not  transferable accross GPUs
	\end{itemize}
...
\begin{vbframe}{Teaching evaluation}

  \beamergotobutton{\href{https://www.lehrevaluation.uni-muenchen.de/evasys/public/online/index}{teaching
evaluation: QAEQ7}}

  	\includegraphics[width = 5cm]{./figure/tmp,qr.png} \\ 
