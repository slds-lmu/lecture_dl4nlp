...
\begin{vbframe}{Scaling Laws: ``Inefficient convergence''}
...
\begin{vbframe}{Compute-Optimal LLM\MakeLowercase{s} (2)}
  ...
  \item We find that all three methods predict
that \myblue{current large models should be substantially smaller and therefore
  trained much longer than is currently done}.
...
\item Not only does Chinchilla outperform its much larger counterpart,
Gopher, but its \myblue{reduced model size reduces inference cost considerably
and greatly facilitates downstream uses on smaller
hardware}.
...
\begin{vbframe}{Sutskever on scaling}
...
\begin{vbframe}{Train/test tradeoff (Andrew Ng)}
...
%\item huge openai models may not make economic sense
	...
	years,
        according to Anthropic’s Dario Amodei.
        \item Rising costs
	could
        lead companies to reallocate their gargantuan
	training
        budgets and focus on more
	cost-effective,
        application-specific approaches. % (Andrew Ng, 2025)
...
\item Phi4: released by Microsoft Dec 2024
	...
	\item pretraining data: 9.8 trillion tokens (big!)
...
\begin{vbframe}{Another scaling law:\\ Context-utilization scaling}

\vfill

\begin{itemize}
\item Input: Very long context, e.g., a book or several
  books
  \item Task: Answer a question that 
 requires combining several pieces of evidence far
    apart in the context
\item The model has to find, retain and integrate
  the relevant parts.

  \end{itemize}

\citebutton{NoLiMa}{https://proceedings.mlr.press/v267/modarressi25a.html}
...
\begin{vbframe}{Jen-Hsun Huang on scaling}

\vfill

\begin{figure}
	\centering
	\includegraphics[height = 6cm]{./figure/jensen,huang,scaling.png} \\ 
\end{figure}
...
\begin{itemize}
	\item The original scaling laws were exclusively
        about \myblue{pretraining}: What is the best model
...
even if a trillion parameter model is the optimal
        ...
        model may be preferable since it is cheaper at
        inference time.
